...
int convert_raw_file(const char* filename, const char* png_filename, int width, int height)
{
    int x, y;
    uint16_t pixel;
    png_structp png_ptr;
    ...
    row_pointer = (png_bytep)malloc(width * 3 * sizeof(png_byte));
    ...
    for (y = 0; y < height; y += 1)
    {
        ...
        for (x = 0; x < width; x += 1)
        {
            gzread(fp, &pixel, sizeof(uint16_t));
